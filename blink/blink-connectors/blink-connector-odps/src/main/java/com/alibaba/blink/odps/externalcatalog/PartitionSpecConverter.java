...
public class PartitionSpecConverter {
	...
	public static PartitionSpec fromMap(Map<String, String> partSpecMap) {
		...
		java.util.Map<String, String> jPartSpec = JavaConversions.asJavaMap(partSpecMap);
		for (java.util.Map.Entry<String, String> partValue : jPartSpec.entrySet()) {
	...
	public static Map<String, String> toMap(PartitionSpec partSpec) {
		java.util.Map<String, String> partitionMap = new LinkedHashMap<>();
		for (String key : partSpec.keys()) {
			partitionMap.put(key, partSpec.get(key));
		}
		return JavaConverters.mapAsScalaMapConverter(partitionMap)
				.asScala()
